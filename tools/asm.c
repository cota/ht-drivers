...
typedef enum {
	FATAL_RUN_TIME, RUN_TIME, FATAL_SYNTAX, SYNTAX
} ErrorType;
...
void Error(ErrorType et, Atom *atom, char *text) {
	...
	if (error_count++ >= MAX_ERROR_COUNT) {
		fprintf(stderr, "Error reporting suppressed after: %d errors",
				MAX_ERROR_COUNT);
		exit(1);
	}
	...
	if (atom) {
		fprintf(stderr, ": at Line %d.%d\n", (int) atom->LineNumber,
				(int) atom->Position);
		fprintf(stderr, "\t\t");
		for (i = 0; i < atom->Position; i++) {
			fputc('.',stderr);
		}
		fprintf(stderr, "v");
		if ( atom->pbFilePos!=NULL) {
			char *p=atom->pbFilePos;
			fprintf(stderr, "\n\t\t");
			while (*p!=0 && *p!='\n') {
	            char c=*(p++);
	            if ( c == '\t') c=' ';
	...
	if (cp) {
		if (strlen(cp) <= MAX_REGISTER_STRING_LENGTH) {
			...
			for (i = 0; i < REGNAMES; i++)
				if (strcmp(upr, Regs[i].Name) == 0) {
					atom->Type = Register;
					atom->Value = st;
					return 1;
				}
			// Search for a register in a range
			for (i = 0; i < REGNAMES; i++) {
	...
	while (atom) {
		if (atom->Text) {
			printf("Typ:%d Txt:%s Val:%d Pos:%d Lin:%d\n", (int) atom->Type,
					atom->Text, (int) atom->Value, (int) atom->Position,
					(int) atom->LineNumber);
		} else {
			printf("Typ:%d Txt:NULL Val:%d Pos:%d Lin:%d\n", (int) atom->Type,
					(int) atom->Value, (int) atom->Position,
					(int) atom->LineNumber);
...
long GetValue(Operand operand) {
	...
	if (operand.SymbolRef)
		return operand.SymbolRef->Value;
	...
	while (1) {
		...
		case OprINC:
			...
			if (left.SymbolRef) {
				...
				Error(RUN_TIME, NULL,
						"Increment: Attempt to write to a literal");
			break;
		...
		case OprDECR:
			...
			if (left.SymbolRef) {
				...
				Error(RUN_TIME, NULL,
						"Decrement: Attempt to write to a literal");
			break;
		...
		case OprAS:
			...
			if (left.SymbolRef) {
				...
				Error(RUN_TIME, NULL,
						"Becomes Equal: Attempt to write to a literal");
			break;
...
int CompareAtomTypes(AtomType x, AtomType y) {
	...
	if (x == Expression)
		if ((y == Numeric) || (y == Alpha))
			return 1;

	if (y == Expression)
		if ((x == Numeric) || (x == Alpha))
			return 1;

	if ((x == Seperator) && (y == Comment))
		return 1;

	if ((y == Seperator) && (x == Comment))
		return 1;
...
long GetConfirm(Atom *atom, OpCode *opcd, int *expect_sr1, int *expect_sr2,
		int *expect_lit, int *expect_dst) {
	...
	*expect_dst = 0;
	...
	if (opcd->Src1 == AtLiteral)
		*expect_lit = 1;
	if (opcd->Src1 == AtAddress)
		*expect_lit = 1;
	if (opcd->Src1 == AtRegister)
		*expect_sr1 = 1;
	if (opcd->Src2 == AtRegister)
		*expect_sr2 = 1;
	if (opcd->Dest == AtRegister)
		*expect_dst = 1;
	...
	if (*expect_lit) {
		a = Expression;
		if (CompareAtomTypes(atom->Type, a) == 0)
			expct = "Expected: Symbol or literal value";
		else
			do {
				if (atom->Next)
					atom = (Atom *) atom->Next;
	...
	while (atom) {
		...
		case Numeric:
			...
			if (expect_lit) {
				...
				expect_lit = 0;
			} else if (!PushOperand(&operand))
				return error_count;
			if (listing)
				printf(" 0x%x", (unsigned int) atom->Value);
			break;
			...
			if (oprId == OprLBL) {
				if (patom->Type == Alpha) {
					...
					if (!pc) {
						...
						Error(SYNTAX, atom,
								"Code block must be contiguous, one start address only");
					}
				}
			} else if (!PushOperator(&oprId))
				return error_count;
		...
		case Op_code:
			...
			GetConfirm(atom, opcd, &expect_sr1, &expect_sr2, &expect_lit,
					&expect_dst);
		...
		case Register:
			if (expect_sr1) {
				...
				inst->Src2 = atom->Value;
				expect_sr2 = 0;
			} else if (expect_dst) {
				inst->Dest = atom->Value;
				expect_dst = 0;
			}
			if (listing)
				printf(" %s", GetRegisterName(atom->Value));
			break;

		case Open:
		case Open_index:
			oprId = OprSTM;
			if (!PushOperator(&oprId))
				return error_count;
			expect_lit = 0;
			if (listing)
				printf("(");
		...
		case Seperator:
			...
			if (listing) {
				cp = "%";
				if ((atom->Text) && strlen(atom->Text))
					printf("\t %s %s %s", cp, atom->Text, cp);
				if (atom->LineNumber) {
					if (pc)
						printf("\nLn:%03d - Pc:%04X\t", (int) atom->LineNumber
								+ 1, pc);
					else
	...
	if (listing) {
		...
		for (i = 0; i < symbolTableSize; i++) {
			if (symbolTable[i].Defined) {
				printf("%s\t= %d \t[0x%x]\n", symbolTable[i].Name,
						(int) symbolTable[i].Value, (int) symbolTable[i].Value);
			} else {
...
int DisAssemble(ProgramBuf *code) {
	...
	for (i = 0; i < code->InstructionCount; i++) {
		...
		if (fnd) {
			...
			case AtLiteral:
				sprintf(tmp, "0x%X", (int) inst->Src1);
				break;
			...
			case AtRegister:
				sprintf(tmp, "%s", GetRegisterName(inst->Src1));
				break;
			...
			case AtLiteral:
				sprintf(tmp, "0x%X", (int) inst->Src2);
				break;
			...
			case AtRegister:
				sprintf(tmp, "%s", GetRegisterName(inst->Src2));
				break;
			...
			case AtLiteral:
				sprintf(tmp, "0x%X", (int) inst->Dest);
				break;
			...
			case AtRegister:
				sprintf(tmp, "%s", GetRegisterName(inst->Dest));
				break;
...
int WriteObject(ProgramBuf *code, FILE *objFile) {
	...
	for (i = 0; i < code->InstructionCount; i++) {
		...
		fprintf(objFile, "0x%X 0x%X 0x%X 0x%X 0x%X\n", (int) inst->Number,
				(int) inst->Src1, (int) inst->Src2, (int) inst->Dest,
				(int) inst->Crc);
		...
		case ASM:
			...
			if (i == 1) {
				...
				if (stat(ass, &assStat) == -1) {
					fprintf(stderr,
							"asm: Can't get file statistics: %s for read\n",
							ass);
					perror("asm");
					exit(1);
				}
			...
			fprintf(
					stderr,
					"asm: Source file: %s must be supplied as first argument\n",
					argv[i]);
			Help();
			break;
