...
char ln[LN], enm[LN];
unsigned long ul,  nzct, abtm, rltm, mstm;
unsigned int sec, msc,  pay;
   ...
   fprintf(assFile,"#include <ctfdefs.h>\n");

   while (fgets(ln,LN,tblFile)) {

      if (isdigit((int) ln[0])) {
	 sscanf(ln,"%u %u %s %x",&sec,&msc,enm,&pay);
	 if      (strcmp(enm,"MEAS") == 0) ul = 0x43040000;
	 else if (strcmp(enm,"PROD") == 0) ul = 0x43030000;
	 else                              ul = TgvGetFrameForName((TgvName *) enm);
	 ...
	 mstm  = rltm % 10;
	 if (rltm) {
	    if (nzct) fprintf(assFile,"   wrlv %d RegNzcFree\n",(int) nzct);
	    if (mstm) fprintf(assFile,"   wrlv %d MSFR\n",(int) rltm);
	 }

	 if (ul) {
	    ul &= 0xFFFF0000;
	    ul |= (pay & 0xFFFF);
	    fprintf(assFile,"   movv 0x%08X EVOUT # Time:%d\n",(int) ul,(int) abtm);
	 } else {
	    fprintf(stderr,"cmt: Don't understand: %s\n",ln);
	    return;
	 }
      }
   }
