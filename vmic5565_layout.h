...
typedef enum {
   ...
   VmicLcsrRX_OVERFLOW       = 0x00000020, //!< Receive buffer has overflowed (RO)
   VmicLcsrRX_ALMOST_FULL    = 0x00000010, //!< Receive buffer is almost full (RO)
   VmicLcsrSYNC_LOST         = 0x00000008, //!< Lost sync clock (RO)
   VmicLcsrSIGNAL_DETECT     = 0x00000004, //!< Incomming signal (light) present (RO)
   VmicLcsrDATA_LOST         = 0x00000002, //!< Data is bad or has been lost (RO)
...
/*! @name VMIC RFM Registers LISR (Status/Control) and LIER (Enable) bits 
 */
//@{
...
typedef enum {
   VmicLisrAC_FLAG           = 0x8000, //!< Auto clear Interrupt: This bit must always be On
   VmicLisrENABLE            = 0x4000, //!< Global Interrupt Enable
   VmicLisrPARITY_ERROR      = 0x2000, //!< Parity error
   VmicLisrWRITE_ERROR       = 0x1000, //!< Cant write a short or byte if parity on
   VmicLisrLOST_SYNC         = 0x0800, //!< PLL unlocked, data was lost, or signal lost
   ...
   VmicLisrROGUE_CLOBBER     = 0x0040, //!< This rogue master has clobbered a rogue packet
   VmicLisrBIT5              = 0x0020, //!< Reserved
   VmicLisrBIT4              = 0x0010, //!< Reserved
   VmicLisrRESET_RQ          = 0x0008, //!< Reset me request from some other node
...
/*! @name Valid enable bits mask in LIER are 1011 1111 1100 1111
 */
//@{
#define VmicLierMASK 0xBFCF

typedef enum {
   VmicLierAC_FLAG           = 0x8000, //!< Auto clear Interrupt: This bit must always be On 
   VmicLierBIT14             = 0x4000, //!< Reserved
   VmicLierPARITY_ERROR      = 0x2000, //!< Parity error
   VmicLierWRITE_ERROR       = 0x1000, //!< Cant write a short or byte if parity on
   VmicLierLOST_SYNC         = 0x0800, //!< PLL unlocked, data was lost, or signal lost
   ...
   VmicLierROGUE_CLOBBER     = 0x0040, //!< This rogue master has clobbered a rogue packet
   VmicLierBIT5              = 0x0020, //!< Reserved
   VmicLierBIT4              = 0x0010, //!< Reserved
   VmicLierRESET_RQ          = 0x0008, //!< Reset me request from some other node
...
//@{
typedef struct {
   u_int8_t  BRV;      //!< 0x00: Board Revision RO
   u_int8_t  BID;      //!< 0x01: Board ID = 65  RO
   u_int16_t SHORT02;  //!< 0x02: Reserved
   u_int8_t  NID;      //!< 0x04: Node ID 1..256 RO
   u_int8_t  BYTE05;   //!< 0x05: Reserved
   u_int16_t SHORT06;  //!< 0x06: Reserved
   u_int32_t LCSR1;    //!< 0x08: Local control and Status 1
   u_int32_t LONG0C;   //!< 0x0C: Reserved
   u_int32_t LISR;     //!< 0x10: Local Interrupt Status Register
   u_int32_t LIER;     //!< 0x14: Local Interrupt Enable Register
   u_int32_t NTD;      //!< 0x18: Network Target Data
   u_int8_t  NTN;      //!< 0x1C: Network Target Node
   u_int8_t  NIC;      //!< 0x1D: Network Interrupt Command
   u_int16_t SHORT1E;  //!< 0x1E: Reserved
   u_int32_t ISD1;     //!< 0x20: Int-1 Sender Data
   u_int8_t  SID1;     //!< 0x24: Int-1 Sender ID
   u_int8_t  BYTE25;   //!< 0x25: Reserved
   u_int16_t SHORT26;  //!< 0x26: Reserved
   u_int32_t ISD2;     //!< 0x28: Int-2 Sender Data
   u_int8_t  SID2;     //!< 0x2C: Int-2 Sender ID
   u_int8_t  BYTE2D;   //!< 0x2D: Reserved
   u_int16_t SHORT2E;  //!< 0x2E: Reserved
   u_int32_t ISD3;     //!< 0x30: Int-3 Sender Data
   u_int8_t  SID3;     //!< 0x34: Int-3 Sender ID
   u_int8_t  BYTE35;   //!< 0x35: Reserved
   u_int16_t HORT36;  //!< 0x36:  Reserved
   u_int32_t INITD;    //!< 0x38: Initialized Node Data
   u_int8_t  INITN;    //!< 0x3C: Initialized Node ID
   u_int8_t  BYTE3D;   //!< 0x3D: Reserved
   u_int16_t SHORT3E;  //!< 0x3E: Reserved
