...
{
	if(carrier_lun < 0 || carrier_lun >= VMOD12A2_MAX_MODULES){
		fprintf(stderr, "libvmod12a22dioaio : Invalid lun %d\n", carrier_lun);
		return -1;
	}
	return carrier_lun;
}
...
int vmod12a2_convert(int fd, int channel, int datum)
{
	...
	ret = DioChannelWrite(IocVMOD12A2, fd, channel, 2, datum); 
	if (ret < 0){
		DioGetErrorMessage(ret, &error);
		fprintf(stderr, "libvmod12a22dioaio : %s\n", error);
	}
	return ret;
}
